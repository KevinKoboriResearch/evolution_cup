...
export const onNewOrder = functions.firestore.document("companies/tapanapanterahs/orders/{orderId}").onCreate(async (snapshot, context) => {
    const orderId = context.params.orderId;

    const querySnapshot = await admin.firestore().collection("users").where('companiesAdmin', 'array-contains', "tapanapanterahs").get();
...
export const onOrderStatusChanged = functions.firestore.document("companies/tapanapanterahs/orders/{orderId}").onUpdate(async (snapshot, context) => {
